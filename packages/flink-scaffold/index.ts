...
const scaffoldRepo = async () => {
    ...
    if (response.handlers.length > 0) {
        ...
        if (permission == "custom") {
            permission = (await prompts([{ type: "text", name: "permission", message: "Required permission" }])).permission;
        }
    }
    ...
    if (response.handlers.includes("get")) {
        let code = "   const item = await ctx.repos." + repoName + "Repo.getById(req.params.id);\n\n";
        code += "   if(item == null){\n";
...
function generateHandler(
    ...
    var fileNameBase = module + "/" + methodName + module + action;
