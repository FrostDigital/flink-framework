...
const main = async () =>{
    ...
    if(!fs.readFileSync("./package.json").toString().includes("@flink-app/flink")){
        console.log(chalk.red("Folder seams to not be a flink project?"));
        process.exit();
    }
...
const scaffoldRepo = async () => {
    const response = await prompts([
        ...
        { type : "list", name : "schema", message : "Schema properties (comma separated)", separator : ","},    
        { type : "multiselect", name : "handlers", message : "Generate handlers",  choices : [ { value : "create", title : "Create"} , { value : "get", title : "Get item"} , { value : "update", title : "Update item"} , { value : "list", title : "List items"} , { value : "delete", title : "Delete item"}]},
    ])    
    let permission = "no";
    if(response.handlers.length > 0){
        permission = (await prompts({ type : "select", name : "permission", message : "Require permission for Handlers?", choices : [ { value : "no", title : "no"}, { value : "authenticated", "title" : "authenticated" } , { value : "custom", "title" : "custom" } ]},        )).permission
        if(permission == "custom"){
            permission = (await prompts([
    ...
    outSchema+=schema.filter((p : string )=>p.length > 0).map((p : string)=>{ return "   " + p + ": string;"}).join("\n");
    outSchema += '\n}'
    ...
    console.log(chalk.magenta("   Creating ./src/repos/" + objectName + "Repo.ts"))
    let outRepo = 'import { FlinkRepo } from "@flink-app/flink";\n';
    outRepo += 'import { Ctx } from "../Ctx";\n';
    outRepo += 'import { ' + objectName +' } from "../schemas/' + objectName +'";\n\n';
    outRepo += 'class ' + objectName + 'Repo extends FlinkRepo<Ctx, ' + objectName + '> {};\n\n';
    ...
    for(let i = 0;i<rows.length;i++){
        ...
        if(!importInjected && !row.startsWith("import ")){
            ...
            newRows.push('import ' + objectName + 'Repo from "./repos/' + objectName + 'Repo";' );
            importInjected = true;
        }
    ...
    if(reposInjected && importInjected){
        ...
        console.log(chalk.green("      Successfully injected repo to ./src/Ctx.ts"));
    }

    if(response.handlers.includes("create")){
        let code = 'const resp = await ctx.repos.'+ repoName  + 'Repo.create(req.body);\n\n'
        code+= '   return {\n';
        code+= '       data: resp,\n';
        code+= '       status : 200,\n';
        code+= '   };\n';
        ...
        let reqCode = 'import { ' + objectName + ' } from "../' + objectName +'";\n\n';
        reqCode+='export interface %Name% extends Omit<' + objectName +', "_id"> {}\n'

        let resCode = 'import { ' + objectName + ' } from "../' + objectName +'";\n\n';
        resCode+='export interface %Name% extends ' + objectName +' {}\n'
        
        generateHandler(objectName, "", "", "post", permission, [], [], code, reqCode, resCode);
    }   

    if(response.handlers.includes("list")){
        let code = '   const items = await ctx.repos.'+ repoName  + 'Repo.findAll({});\n\n'
        code+= '   return {\n';
        code+= '       data: { items },\n';
        code+= '       status : 200,\n';
        code+= '   };\n';
        ...
        generateHandler(objectName, "", "", "get", permission, [], [], code, "", resCode);
    }   

    if(response.handlers.includes("get")){
        let code = '   const item = await ctx.repos.'+ repoName  + 'Repo.getBydId(req.params.id);\n\n'
        code+= '   if(item == null){\n';
        ...
        generateHandler(objectName, "", "id", "get", permission, [], [], code, "", resCode);
    }   
    

    if(response.handlers.includes("delete")){
        let code = '   const items = await ctx.repos.'+ repoName  + 'Repo.deleteById(req.params.id);\n\n'
        code+= '   if(items == 0){\n';
        code+= '       return notFound();\n';
        code+= '     }\n\n';
        code+= '   return {\n';
        ...
        code+= '       status : 200,\n';
        code+= '   };\n';
        generateHandler(objectName, "", "id", "delete", permission, [], [], code, "", "");
    }   
    

    if(response.handlers.includes("update")){
        let code='   const item = await ctx.repos.'+ repoName  + 'Repo.updateOne(req.params.id, req.body);\n\n'
        code+= '   if(item == null){\n';
        code+= '       return notFound();\n';
        code+= '     }\n\n';
        ...
        let reqCode = 'import { ' + objectName + ' } from "../' + objectName +'";\n\n';
        reqCode+='export interface %Name% extends Partial<Omit<' + objectName +', "_id">> {}\n'

        let resCode = 'import { ' + objectName + ' } from "../' + objectName +'";\n\n';
        resCode+='export interface %Name% extends ' + objectName +' {}\n'

        
        generateHandler(objectName, "", "id", "put", permission, [], [], code, "", resCode);
    }   
...
const scaffoldHandler = async () => {
    const response = await prompts([
        ...
        { type : "text", name : "action", message : "Object or action name (eg. user)" },
    ...
    generateHandler(response.module, response.action, response.parameter, response.method, permission, response.reqParameters, response.resValues, "", "", "" )



}

function generateHandler(_module : string, _action : string, _parameter : string, _method : string, _permission : string, _reqParameters : string[], _resValues : string[], _code : string, _reqSchemaCode : string, resSchemaCode : string){
    ...
    var out = 'import { Handler, HttpMethod, notFound, RouteProps } from "@flink-app/flink";\n';
    out+= 'import { Ctx } from "../../Ctx";\n'
    out+= 'import { ' + reqSchemaName + ' } from "../../schemas/'  + fileNameBase + 'Req";\n'
    out+= 'import { ' + resSchemaName + ' } from "../../schemas/'  + fileNameBase + 'Res";\n\n'
    ...
    out+='const ' + objBase + ': Handler<Ctx, ' + reqSchemaName + ', ' + resSchemaName + ', Params> = async ({ ctx, req }) => {\n\n'
    ...
    console.log(chalk.magenta("   Creating ./src/handlers/" + fileNameBase + ".ts"))
    fs.ensureFileSync("./src/handlers/" + fileNameBase + ".ts");
    fs.writeFileSync("./src/handlers/" + fileNameBase + ".ts", out);
    

    console.log(chalk.magenta("   Creating ./src/schemas/" + fileNameBase + "Req.ts"))
    var outReq = 'export interface ' + reqSchemaName + '{\n';
    outReq+=_reqParameters.filter((p)=>p.length > 0).map((p)=>{ return "   " + p + ": string;"}).join("\n");
    outReq += '\n}'
    if(_reqSchemaCode != "") outReq = _reqSchemaCode.replace("%Name%", reqSchemaName);
    fs.ensureFileSync("./src/schemas/" + fileNameBase + "Req.ts");
    fs.writeFileSync("./src/schemas/" + fileNameBase + "Req.ts", outReq);


    console.log(chalk.magenta("   Creating ./src/schemas/" + fileNameBase + "Res.ts"))
    var outRes = 'export interface ' + resSchemaName + '{\n';
    outRes+=_resValues.filter((p)=>p.length > 0).map((p)=>{ return "   " + p + ": string;"}).join("\n");
    outRes += '\n}'
    if(_reqSchemaCode != "") outRes = _reqSchemaCode.replace("%Name%", resSchemaName);
